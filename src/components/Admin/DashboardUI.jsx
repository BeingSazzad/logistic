        ...
        {[
          ...
          <div key={i} className="card p-5 border-transparent hover:border-gray-200 transition-all hover:shadow-md">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className={`text-xs font-black uppercase text-${m.color}-600 bg-${m.color}-50/50 px-2 py-1 rounded-full`}>{m.trend}</span>
            </div>
            <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 px-2">
        ...
        <div className="card overflow-hidden flex flex-col min-h-[440px] shadow-sm">
          <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-[#FAFAFA]">
            <div>
              <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest">Load Throughput</h3>
              <p className="text-xs text-gray-400 font-bold uppercase mt-1">Monthly Analytics (Volume)</p>
            </div>
            <select value={LoadYear} onChange={e => setLoadYear(e.target.value)} className="bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-xs font-black outline-none shadow-sm cursor-pointer hover:border-brand">
            ...
            {LoadData.map((v, i) => (
              <div key={i} className="flex-1 flex flex-col justify-end gap-3 group/bar h-full">
                <div className="relative flex-1 flex flex-col justify-end">
                  ...
                  >
                    <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-gray-900 text-brand text-xs font-black px-1.5 py-0.5 rounded opacity-0 group-hover/bar:opacity-100 shadow-xl z-20 transition-all">
                      {v}k
                    </div>
                  </div>
                </div>
                <span className="text-xs font-black text-gray-400 text-center uppercase tracking-tighter">{months[i]}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {incomeData.map((v, i) => (
              <div key={i} className="flex-1 flex flex-col justify-end gap-3 group/bar h-full">
                <div className="relative flex-1 flex flex-col justify-end">
                  ...
                  >
                    <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-gray-900 text-emerald-400 text-xs font-black px-1.5 py-0.5 rounded opacity-0 group-hover/bar:opacity-100 shadow-xl z-20 transition-all">
                      ${v}k
                    </div>
                  </div>
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="card h-full flex flex-col overflow-hidden bg-white border-2 border-brand/20 shadow-lg shadow-brand/5">
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-100 flex items-center justify-center">
              <button onClick={() => navigate('/admin/audit')} className="text-xs font-black text-gray-400 hover:text-brand uppercase tracking-widest transition-colors flex items-center gap-2">
        ...
        <div className="lg:col-span-4 card p-8 flex flex-col justify-between shadow-sm">
          <div>
            <div className="flex justify-between items-center mb-8">
              ...
              <span className="text-xs font-black text-gray-400 uppercase tracking-tighter bg-gray-50 px-3 py-1 rounded">Daily Mix</span>
            </div>
              ...
              {recentActivities.slice(0, 4).map((act, i) => (
                <div key={i} className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-black text-gray-900 truncate uppercase tracking-wide leading-none mb-1">{act.action}</p>
