            ...
            {threads.map(t => (
              ...
              >
                ...
                <div className="flex justify-between items-center">
                  ...
                  {t.unread > 0 && (
                    <span className="w-5 h-5 rounded-md bg-[#111] text-[#FFCC00] flex items-center justify-center text-[10px] font-black shrink-0 shadow-sm">{t.unread}</span>
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-[#FAFAFA] flex flex-col gap-5">
            ...
            {activeMessages.map((m) => (
               <div key={m.id} className={`flex flex-col gap-1.5 max-w-[70%] ${m.sender === 'dispatch' ? 'self-end' : ''}`}>
                  ...
                  <span className={`text-[9px] font-bold text-gray-400 uppercase tracking-widest ${m.sender === 'dispatch' ? 'text-right mr-1' : 'ml-1'}`}>{m.time} {m.sender === 'dispatch' ? '✓✓' : ''}</span>
               </div>
            ))}
          </div>
